...
import { updateListing } from '@/lib/listing-actions'
import { useRouter } from 'next/navigation'
import {
  Upload, X, Check, Star, ChevronLeft,
  Ruler, Tag, Camera,
} from 'lucide-react'
...
function NumericInput({
  value, onChange, placeholder, hasError, step,
}: {
  value: string
...
export function EditListingForm({
  ...
  const [form, setFormState] = useState<EditFormData>({
    ...
    floorLevel:     listing.floor_level ?? 'ground',
    sizeGlaSqm:     String(listing.gla_sqm ?? listing.size_sqm ?? ''),
    facadeMeters:   String(listing.facade_meters ?? ''),
    ceilingHeight:  String(listing.ceiling_height ?? ''),
    amenities:      listing.amenities ?? [],
    rentalTypes:    listing.rental_types ?? [],
    pricePerSqm:    String(listing.price_per_sqm ?? ''),
    priceMonthly:   String(listing.price_monthly ?? ''),
    utilitiesMonthly: String(listing.utilities_monthly ?? ''),
    longTermNotes:  listing.long_term_notes ?? '',
    popupPriceAmount: String(listing.price_daily_popup ?? ''),
    popupPriceUnit:   listing.popup_price_unit ?? 'day',
    marketingPriceAmount: String(listing.price_daily_marketing ?? ''),
    marketingPriceUnit:   listing.marketing_price_unit ?? 'day',
    availableFromImmediate: !listing.available_from,
    availableFrom:  listing.available_from ?? new Date().toISOString().split('T')[0],
    availableUntil: listing.available_until ?? '',
  })

  const [photos, setPhotos] = useState<PhotoItem[]>(
    (listing.images ?? []).map((url) => ({ kind: 'existing' as const, url }))
  )
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      for (const photo of photos.filter((p) => p.kind === 'new') as Extract<PhotoItem, { kind: 'new' }>[]) {
      ...
      // Rebuild imageUrls preserving the user's chosen order
      let uploadIdx = 0
      ...
      <div className="bg-white rounded-3xl border border-warm-border p-8">
        ...
        {step === 1 && (
          <div className="space-y-5">
            <div className="mb-6">
              <p className="text-xs font-semibold uppercase tracking-widest text-ink-subtle mb-1">Step 1 of 3</p>
        ...
        {step === 2 && (
          <div className="space-y-3">
            <div className="mb-6">
              <p className="text-xs font-semibold uppercase tracking-widest text-ink-subtle mb-1">Step 2 of 3</p>
              <h2 className="text-xl font-bold text-ink">Pricing & availability</h2>
            </div>

            {errors.rentalTypes && <p className="text-xs text-red-500">{errors.rentalTypes}</p>}
                        ...
                        <>
                          ...
                          <Field>
                            ...
                            <textarea className={`${inputCls()} resize-none`} rows={2}
                              value={form.longTermNotes} onChange={(e) => set('longTermNotes', e.target.value)}
                      ...
                      {rt.value === 'popup' && (
                        <Field>
                          ...
                          <div className="flex gap-2">
                            ...
                            <select value={form.popupPriceUnit} onChange={(e) => set('popupPriceUnit', e.target.value)}
                      ...
                      {rt.value === 'marketing' && (
                        <Field>
                          ...
                          <div className="flex gap-2">
                            ...
                            <select value={form.marketingPriceUnit} onChange={(e) => set('marketingPriceUnit', e.target.value)}
                              className="px-3 py-3 rounded-xl border border-warm-border bg-white text-ink text-sm focus:outline-none focus:ring-2 focus:ring-forest/10 focus:border-forest/50">
                              <option value="day">€ / day</option>
                              <option value="week">€ / week</option>
        ...
        {step === 3 && (
          <div className="space-y-5">
            <div className="mb-6">
              <p className="text-xs font-semibold uppercase tracking-widest text-ink-subtle mb-1">Step 3 of 3</p>
              <h2 className="text-xl font-bold text-ink">Photos</h2>
              <p className="text-sm text-ink-muted mt-1">
                The first photo is your cover. Click the <Star className="inline h-3.5 w-3.5 text-amber-500" /> to rearrange.
              </p>
            </div>
                ...
                {photos.map((photo, i) => {
                  ...
                  return (
                    <div key={i} className="relative aspect-square rounded-2xl overflow-hidden group">
                      ...
                      {i === 0 && (
                        <div className="absolute bottom-2 left-2 bg-forest text-white text-[10px] font-bold px-2 py-0.5 rounded-full">Cover</div>
                      )}
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()}
              ...
              className="w-full border-2 border-dashed border-warm-border hover:border-forest/40 rounded-2xl p-8 flex flex-col items-center gap-3 transition-colors group"
            >
              <div className="w-10 h-10 rounded-2xl bg-forest-light flex items-center justify-center group-hover:scale-105 transition-transform">
                <Upload className="h-4 w-4 text-forest" />
              </div>
              <p className="text-sm font-semibold text-ink">{photos.length > 0 ? 'Add more photos' : 'Upload photos'}</p>
            </button>
